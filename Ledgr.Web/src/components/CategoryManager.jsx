    ...
    <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-4">
      <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl w-full max-w-md flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto flex-1 px-5 py-3">
          {categories.length === 0 && (
            <p className="text-[#555] text-sm text-center py-4">No categories yet.</p>
          )}
          {categories.map(cat => (
            <div
              key={cat.id}
              className={`flex items-center justify-between rounded-lg px-3 py-3 border mb-2 transition-colors ${editing?.id === cat.id ? 'bg-indigo-500/10 border-indigo-500/40' : 'bg-[#111] border-[#2a2a2a]'}`}
            >
              ...
              <div className="flex gap-1 shrink-0 ml-3">
                <button
                  className="px-3 py-1.5 rounded-md text-xs text-[#aaa] bg-transparent border border-[#333] cursor-pointer hover:border-[#555] hover:text-white transition-colors"
                  onClick={() => editing?.id === cat.id ? reset() : startEdit(cat)}
                >
                  {editing?.id === cat.id ? 'Cancel' : 'Edit'}
                </button>
                <button
                  className="px-3 py-1.5 rounded-md text-xs text-red-400 bg-transparent border border-red-900 cursor-pointer hover:bg-red-900/20 transition-colors"
